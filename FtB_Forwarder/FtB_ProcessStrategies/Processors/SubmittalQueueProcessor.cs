...
namespace FtB_ProcessStrategies
{
    public class SubmittalQueueProcessor
    {
        ...
        public async Task<IEnumerable<SendQueueItem>> ExecuteProcessingStrategy(SubmittalQueueItem submittalQueueItem)
        {
            ...
            {
                _log.LogError(rfEx, $"{GetType().Name}: An error occured while processing: {0}", submittalQueueItem?.ArchiveReference);
            ...
            {
                _log.LogError(ex, $"{GetType().Name}: An error occured while processing: {0}", submittalQueueItem?.ArchiveReference);
                throw;
            }
